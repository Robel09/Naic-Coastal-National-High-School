...
if(isset($_REQUEST["score_ID"])){
   ...
   $stmt3 = $auth_user->runQuery("SELECT question_ID FROM `room_test_questions` WHERE test_ID = ".$test_ID."");
   $stmt3->execute();
   $qcount = $stmt3->rowCount();

  $stmt4 = $auth_user->runQuery("SELECT * FROM `room_test_attemp` WHERE user_ID = ".$score_user_ID." AND test_ID = ".$test_ID."");
  $stmt4->execute();
