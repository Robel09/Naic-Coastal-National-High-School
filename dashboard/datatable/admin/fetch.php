...
foreach($result as $row)
{
		...
		if(empty($row["user_ID"]))
	{
		...
		$acreg = "UN";
		$btnrg = '
		 <button type="button" class="btn btn-success btn-sm gen_account" id="'.$row["rad_ID"].'"><i class="icon-key" style="font-size: 20px;"></i></button>';
		 // <a class="dropdown-item gen_account"  id="'.$row["rad_ID"].'">Generate Account</a>
	}
	else
	{
	...
	$sub_array = array();
		...
		//     <a class="dropdown-item view"  id="'.$row["rad_ID"].'">View</a>
		//     <a class="dropdown-item edit"  acreg="'.$acreg.'"  id="'.$row["rad_ID"].'">Edit</a>
		//       '.$btnrg.'
