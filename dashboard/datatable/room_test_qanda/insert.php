...
if(isset($_POST["q_operation"]))
{
	...
	if($_POST["q_operation"] == "QandA_edit")
	{
		...
		**/
		$x = 0;
		$stmt = $room->runQuery("SELECT * FROM `room_test_choices` WHERE question_ID =".$question_ID."");
		$stmt->execute();
		...
		SET NEW QUESTION;
		**/
		$stmt00 = $room->runQuery("UPDATE `room_test_questions` SET `question` = '".$q_question."' WHERE `room_test_questions`.`question_ID` =".$question_ID.";");
		$stmt00->execute();
		...
		**/
		...
		$stmt1 = $room->runQuery("UPDATE `room_test_choices` 
				SET `is_correct` = '0' 
		...
		**/
		...
		$stmt2 = $room->runQuery("UPDATE `room_test_choices` SET `is_correct` = '1' WHERE `room_test_choices`.`choice_ID` = ".$new_is_correct.";");
	...
	if($_POST["q_operation"] == "retake")
	{
		...
		$stmt1 = $room->runQuery("UPDATE `room_test_attemp` SET `count` = '".$new_count."' WHERE `room_test_attemp`.`atmp_ID` = ".$atmp_ID."");
		$stmt1->execute();
