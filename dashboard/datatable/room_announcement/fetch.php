...
foreach($result as $row)
{
		...
		if($_SESSION['user_ID'] === $row["user_ID"]){
			...
			<button type="button" class="btn btn-primary btn-sm edit"  user-id="'.$row["user_ID"].'" id="'.$row["post_ID"].'">Edit</button>

			';
			$delete_by_user_who_posted='
			<button type="button" class="btn btn-danger btn-sm delete"  id="'.$row["post_ID"].'">Delete</button>
			';
		}
		else{
			$edit_by_user_who_posted='';
